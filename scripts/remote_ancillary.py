...
def receive_params(input_owner, cache):
    """Receives parameters from the input dict of owner site and puts them in cache dict.
...
class Stats_agg():
    """Stats of the whole data, an aggregation of the stats from the local sites.
    ...
    Attributes:
        ...
        train_index: dict, {local site (str): index of X for X_train (list)}, for reconstruction of the train/test datasets after the program ends.  
        test_index: dict, {local site (str): index of X for X_test (list)}  
    """

    def __init__(self, preprocess_method, n_features):
        """Initiates Stats_agg with preprocess_method (str) and n_features (int).
        """        
    ...
    def agg_stats(self, input):
        """Aggregates stats from the local sites.
...
class Stats_agg_CV():
    """Stats of the whole data for cross validation, an aggregation of the stats from the local sites.

    Attributes:
        n_samples_folds: int list, len = n_folds, the number of samples in the training dataset of each fold.
        n_samples_valids: int list, len = n_folds, the number of samples in the validation dataset of each fold.
        mean_X_folds: float list of 2D-list, shape (n_folds, (n_features,)), column-wise mean of X_fold.
        ...
        fold_indices: dict, {local site (str): index of X_train for X_fold (list)}, for reconstruction of the fold/valid datasets after the program ends.  
        valid_indices: dict, {local site (str): index of X_train for X_valid (list)}  
    """

    def __init__(self, preprocess_method, n_features, n_folds):
        """Initiates Stats_agg_CV with preprocess_method (str), n_features (int) and n_folds (int).
        """          
...
def agg_scale_sqsum(input, preprocess_method, n_features, n_samples):
    """Aggregates sum of squares of X_train from the local sites, and set sum of squares and scale of the whole X_train.

    Sum of sqaures of the whole data will be used in coordinate descent.
    Scale of the whole data will be used in preprocessing and setting w and intercept.
    ...
    Returns:
        ...
        sqsum_X_train: list, len = n_features, column-wise sum of squares of X_train.
    """
...
def agg_scale_sqsum_CV(input, preprocess_method, n_features, n_samples_folds, n_folds):
    """Aggregates sum of squares of X_fold for cross validation from the local sites, and set sum of squares and scale of the whole X_fold.
    ...
    Args:
        ...
        preprocessed_method: str, preprocessing method applied on the training dataset.        
        n_features: int, the number of features.  
        n_samples_folds: int list, len = n_folds, the number of samples in X_fold. 
        n_folds: int, the number of folds in cross validation.

    Returns:
        scale_X_folds: a list of lists, shape (n_folds, (n_features,)), column-wise scale of X_fold.
        sqsum_X_folds: a list of lists, shape (n_folds, (n_features,)), column-wise sum of squares of X_fold.
    """    
    sqsum_X_folds = []   
...
def set_lambdas(lambdas, input, cache):
    ...
    If the user-input lambdas is empty, generates one in decreasing order. Else, sorts it in decreasing order.
...
def lambda_grid(Xy, eps, n_lambdas, n_samples):
    ...
    In the grid, the max is lambda_max (i.e., if lambda >= lambda_max, w = 0) which will be automatically calculated.
...
def pick_jth_feature(selection, n_features, i_feature):        
    """Pick next feature whose coefficient will be updated in coordinate descent.

    Args:
        selection: str, "random" or "cyclic", the sequence of updating coefficients in an iteration.
        n_feature: int, the number of features.
        i_feature: int, if selection == "cyclic", then next feature is the i_feature-th feature.
