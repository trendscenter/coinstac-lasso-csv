...
def readin(args):
    ...
    Raises:
        ValueError: if train_split_owner >= 1 and train_split_local >= 1,  
                    all train no test data for the whole.
        ValueError: if train_split_owner <= 0 and train_split_local <= 0, 
                    all test no train data for the whole.
    """
    input = args["input"]
    state = args["state"]
    cache = args["cache"]
...
def split_save_train_test(input, output, cache, id, owner, cache_dir, X, y):
    ...
    if (
        train_split_owner >= 1 and train_split_local >= 1
    ):  # all train no test data
        raise ValueError("No test data, adjust train_split_owner/local")
    elif (
        train_split_owner <= 0 and train_split_local <= 0
    ):  # all test no train data
...
def split_folds_save_valid(input, output, cache, cache_dir, X_train, y_train):
    ...
    Returns:
        fold_indices: a list of lists, len = n_folds, indices of X/y_train for 
            the training dataset in each fold.
        valid_indices: a list of lists, len = n_folds, indices of X/y_train for
            the validation dataset in each fold. 
    """
    if cache["train_split"] > 0:  # has train data
        ...
        for ii, (fold_index, valid_index) in enumerate(kf.split(X_train)):
            ...
            np.save(
                os.path.join(cache_dir, "X_valid_" + str(ii) + ".npy"),
                X_train[valid_index],
            )
            np.save(
                os.path.join(cache_dir, "y_valid_" + str(ii) + ".npy"),
                y_train[valid_index],
            )
    else:
...
class Stats:
    ...
    Attributes:
        ...
        n_samples_test: int.
        sum_X_train: float np.ndarray of shape (n_features,), column-wise sum 
            of X_train.
...
class Stats_CV:
    """Summation stats of fold/valid datasets for CV at the local site.

    Attributes:
        n_samples_folds: int list, len = n_folds, the number of samples in the 
            training dataset in each fold.
        n_samples_valids: int list, len = n_folds, the number of samples in the 
            validation dataset in each fold.
        sum_X_folds: float list of 2D-list, shape (n_folds, (n_features,)), 
            column-wise sum of X_fold.
    ...
    def __init__(self, cache, preprocess_method):
        ...
        self.sum_X_folds = np.tile(
            np.zeros(n_features), (n_folds, 1)
        ).tolist()  # a list of lists, not for "none"
    ...
    def cal_stats(self, X_train, y_train, fold_indices, valid_indices):
        ...
        Args:
            X_train: np.ndarray of shape (n_samples, n_features).
            y_train: np.ndarray of shape (n_features,).   
            fold_indices: a list of lists, len = n_folds, indices of X/y_train 
                for the training dataset in each fold.
            valid_indices: a list of lists, len = n_folds, indices of X/y_train
                for the validation dataset in each fold.         
        """
        for ii, (fold_index, valid_index) in enumerate(
            zip(fold_indices, valid_indices)
        ):
            ...
            if self.preprocess_method != "none":
                self.sum_X_folds[ii] = np.sum(
                    X_train[fold_index], axis=0
                ).tolist()
