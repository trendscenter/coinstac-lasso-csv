...
Each local site: 
    ...
    1. preprocesses the local training data based on the aggregated stats sent by the remote site.
    2. calculates the update based on the local training dataset, sends it to the remote site.
    3. tests on the local testing dataset using the model sent by the remote site, sends performance metrics to the remote site. 

Raises:
    Exception: If the value of "msg" from sys.stdin is neither empty nor in ["to_sqsum", "to_preprocess", "to_train", "to_test"].
"""
import os
import sys
